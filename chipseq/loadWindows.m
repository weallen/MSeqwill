...
function chr_range = findChrIdxRange(windows)
    chr_range = zeros(20, 2);
    % assumes there are 20 chroms (for mice)
    for chr_num=1:20
        chr_start = find(windows.chr == chr_num, 1, 'first');
        chr_end = find(windows.chr == chr_num, 1, 'last');
        chr_range(chr_num,:) = [chr_start, chr_end];
    end
end
